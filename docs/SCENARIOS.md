...
- *Time-to-illiquid*: $\min\{\tau : L_\tau = 0 \;\wedge\; \text{queued}_\tau > 0\}$, or infinity if never;
...
**Shock function $\delta_{\text{S3}}$**: two coupled price paths are
generated.
...
**Shock function $\delta_{\text{S4}}$**: joint shock $(\Delta, \Delta t)$ calibrated at the 95th percentile *jointly*. The 99th-percentile
joint is unreliable on a 12-month sample.

**Behavioural rule $\rho_{\text{S4}}$**:
...
**Shock function $\delta_{\text{S5}}$**: reconstruct the historical
...
**Behavioural rule $\rho_{\text{S5}}$**: as in S4 (endogenous
...
- $\mathrm{LCR}_{\text{on-chain}}(M, t_0, \sigma_{\text{S5}}, h = 24\text{h}) < 100\%$;
- *Time-to-illiquid* $(M, \sigma_{\text{S1, 99th-percentile}}, h = 24\text{h}) < 24$ hours;
- $\Pr[\text{bad debt} > 0 \mid \sigma_{\text{S4}}] > 5\%$,
...
between our $\mathrm{LCR}_{\text{on-chain}}$ ranking and theirs.
...
- Markets with $U_{t_0} < 50\%$ and well-funded suppliers should
  *never* reach illiquidity under $\sigma_{\text{S1}}$ at the
...
|---|---|---|---|
| $\mathrm{LCR}_{\text{on-chain}}$ | float | percentage | green $\geq 150\%$ / yellow $\in [100\%, 150\%)$ / red $< 100\%$ |
