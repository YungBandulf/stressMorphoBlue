...
$\mathrm{LCR}_{\text{on-chain}}(M, t, \sigma, h)$ where $\sigma$ is
the stress scenario and $h$ is the horizon in blocks:

$$\mathrm{LCR}_{\text{on-chain}}(M, t, \sigma, h) \;=\; \frac{L_1(M, t) \;+\; L_{\text{2A,net}}(M, t, \sigma)}{O_\sigma(M, t, h) \;-\; \min\bigl(I_\sigma(M, t, h),\; 0.75 \cdot O_\sigma\bigr)}$$

where:
- $L_1(M, t) = S_t - B_t$ is the on-chain analogue of HQLA Level 1
  (instant liquidity, no haircut);
- $L_{\text{2A,net}}(M, t, \sigma)$ is the per-position liquidation
...
1. The liquidator seizes collateral up to the *liquidation incentive
   ...
   $\text{seized}_i = \min\bigl(b_i \cdot \phi(\Lambda) / P,\; c_i\bigr)$
2. The liquidator sells the seized collateral on a decentralised
   exchange at the realised price $P \cdot (1 - \pi(\text{seized}_i))$;
...
$$r_i \;=\; \min\Bigl(\text{seized}_i \cdot P \cdot (1 - \pi(\text{seized}_i)),\; b_i\Bigr) \;-\; \text{bad-debt}_i$$

where the bad debt for position $i$ is

$$\text{bad-debt}_i \;=\; \max\Bigl(0,\; b_i - \text{seized}_i \cdot P \cdot (1 - \pi(\text{seized}_i))\Bigr).$$

The aggregate Level 2A is

$$L_{\text{2A,net}}(M, t, \sigma) \;=\; \sum_i r_i.$$
...
$$\alpha \;=\; \min\Bigl(0.60,\; \max\bigl(0.05,\; 1.5 \cdot q_{0.99}(\text{drawdowns}) + 0.30 \cdot \mathbf{1}\{q_{0.99}(\text{drawdowns}) > 0.05\}\bigr)\Bigr)$$

where:
- $q_{0.99}(\text{drawdowns})$ is the 99th-percentile empirical
...
- $\mathrm{LCR}_{\text{on-chain}}$ — the primary metric, reported as
